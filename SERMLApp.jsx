...
const CATEGORY_HELP = {
  hate: "Targets protected groups through hateful content.",
  offense: "Signals abusive or insulting language.",
  bully: "Reflects harassment or intimidation patterns.",
  threat: "Expresses direct or implied harm.",
};
...
export default function SERMLApp() {
  ...
  const resultTone = result?.risk_level || "LOW";
    ...
    ? [
        {
          ...
          detail: `${result.risk_level} risk level`,
        },
        {
          label: "Top Category",
          value: topCategory,
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <span
                        className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] ${
                          RISK_BADGE_CLASSES[item.risk_level] || RISK_BADGE_CLASSES.LOW
                        }`}
                      >
                        {item.risk_level}
                      </span>
              ...
              <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
                <div className="max-w-2xl">
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-600">
                    Keep the prompt area concise so the figure foreground stays on the routed result rather than the input form.
                  </p>
                </div>
                <div className="rounded-2xl border border-[#e7dccb] bg-white px-4 py-3 text-xs leading-5 text-slate-500">
                  Suggested thesis crop: header, input card, and the first two result panels.
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-600">
                    The layout below emphasizes the routing narrative first, with raw JSON moved to a secondary expandable section.
                  </p>
                </div>
                <button
                  onClick={copyResult}
                ...
                <div className="mt-6 space-y-6">
                  <div
                    className={`rounded-[28px] border px-5 py-5 ${
                      RISK_PANEL_CLASSES[resultTone] || RISK_PANEL_CLASSES.LOW
                    }`}
                  >
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        <span
                          className={`rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] ${
                            RISK_BADGE_CLASSES[resultTone] || RISK_BADGE_CLASSES.LOW
                          }`}
                        >
                          {result.risk_level}
                        </span>
                  ...
                  <div className="grid gap-6 xl:grid-cols-[1.08fr_0.92fr]">
                    <section className="rounded-[28px] border border-[#e6dccb] bg-white p-5">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className="rounded-full bg-[#edf5f3] px-3 py-1 text-xs font-medium text-[#2f6f73]">
                      ...
                      <div className="mt-5 space-y-4">
                        ...
                        <div className="flex gap-4">
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-sm leading-6 text-slate-600">
                              {result.harmful
                                ? `The pipeline marks the sample as harmful with ${result.risk_level.toLowerCase()} risk.`
                                : "The pipeline marks the sample as safe with no harmful expert label returned."}
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    <section className="rounded-[28px] border border-[#e6dccb] bg-white p-5">
                      ...
                      <p className="mt-1 text-sm text-slate-600">
                        Horizontal confidence bars read better in a report excerpt than a large dashboard chart.
                      </p>
                          ...
                          <div key={`${item.label}-${item.confidence}`} className="rounded-[22px] border border-[#ebe1d0] bg-[#fcf8f2] p-4">
                            <div className="flex items-center justify-between gap-3">
                              <div>
                                ...
                                <p className="mt-1 text-xs text-slate-500">{CATEGORY_HELP[item.label] || "Detected by the routed expert."}</p>
                              </div>
                  ...
                  <details className="rounded-[24px] border border-[#e6dccb] bg-white p-5">
                    <summary className="cursor-pointer text-sm font-semibold text-slate-700">Raw JSON and reproducibility details</summary>
...
{JSON.stringify(result, null, 2)}
                ...
                <div className="mt-6 rounded-[28px] border border-dashed border-[#d7cbb7] bg-white p-8">
                  ...
                  <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-600">
                    For the thesis figure, keep the screenshot focused on the verdict banner, the routing snapshot, and the confidence
                    panel. The raw JSON can stay collapsed unless you want to highlight the underlying schema.
                  </p>
                </div>
              )}
            </section>
